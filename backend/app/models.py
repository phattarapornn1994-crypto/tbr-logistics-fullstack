...
class Product(SQLModel, table=True):
    """ข้อมูลสินค้า/บรรจุภัณฑ์"""
    id: Optional[int] = Field(default=None, primary_key=True)
    code: str = Field(index=True, unique=True)
    name: str
    category: str  # ขวดเก่า, PAPER, PET, UBC, HDPE
    unit: str = "ก้อน"  # ก้อน, กล่อง
    # ขนาด (cm)
    width_cm: float = 0
    length_cm: float = 0
    height_cm: float = 0
    ...
    weight_kg: float = 0
    # สำหรับขวดเก่า
    pieces_per_box: Optional[int] = None  # 12 หรือ 24 ใบต่อกล่อง
    stackable: bool = Field(default=True)  # ใช้ Field(default=True) เพื่อให้มีค่า default
    max_stack: Optional[int] = None  # จำนวนสูงสุดที่ซ้อนได้
    note: Optional[str] = None
