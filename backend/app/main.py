...
from .schemas import (
    ...
    TSPRequest, TSPResult,
    RouteRequest as RouteRequestSchema, RouteResult as RouteResultSchema
)
...
def create_product(payload: ProductIn, session: Session = Depends(get_session)):
    ...
    if product_dict.get("stackable") is None:
        product_dict["stackable"] = True
...
def optimize_plan(payload: PlanIn, session: Session = Depends(get_session)):
    try:
        ...
        raise HTTPException(500, f"Plan optimization failed: {str(e)}")
...
# ---------- Routing APIs ----------
def get_longdo_route(waypoints: List[dict], use_truck: bool = True) -> dict:
    """เรียก Longdo Map Routing API - ใช้ Truck Routing API"""
    ...
    try:
        ...
        # ใช้ Truck Routing API สำหรับรถบรรทุก
        if use_truck:
            # Truck Routing API format: lat,lon|lat,lon|...
            points = "|".join([f"{wp.get('lat', 0)},{wp.get('lng', wp.get('lon', 0))}" for wp in waypoints])
            url = f"https://api.longdo.com/RouteServiceTruck/json/route"
            params = {
                "key": LONGDO_API_KEY,
                "points": points,
                "mode": "t"  # t=fastest with traffic, c=fastest static, d=shortest static, e=shortest with traffic
            }
        else:
            # Standard Route API
            points = "|".join([f"{wp.get('lat', 0)},{wp.get('lng', wp.get('lon', 0))}" for wp in waypoints])
        ...
        if resp.status_code == 200:
            ...
            # Truck Routing API response format
            if use_truck and data and isinstance(data, dict):
            ...
            # Standard Route API response format
            elif not use_truck and data and isinstance(data, dict) and data.get("code") == 0:
                route_data = data.get("data")
                if route_data and isinstance(route_data, list) and len(route_data) > 0:
                    route = route_data[0]
                    if route and isinstance(route, dict):
...
@app.post("/api/routing/route", response_model=RouteResultSchema)
def get_route(req: RouteRequestSchema):
    """เรียก Longdo RouteServiceTruck API ผ่าน backend proxy (แก้ปัญหา CORS)"""
    try:
        result = get_longdo_route(req.waypoints, use_truck=req.use_truck)
...
@app.post("/api/routing/tsp", response_model=TSPResult)
def calculate_tsp(req: TSPRequest):
    """คำนวณ TSP (Travelling Salesman Problem) เพื่อหาลำดับที่ดีที่สุด"""
    LONGDO_API_KEY = "be18ff5b483db0626748b0c4d17ee5a9"
    
    try:
        if not req.lon or not req.lat or len(req.lon) != len(req.lat):
            raise HTTPException(400, "lon and lat arrays must have the same length")
        
        if len(req.lon) < 2:
            raise HTTPException(400, "At least 2 points required for TSP")
        ...
        data = {
            "lon": req.lon,
            "lat": req.lat,
            "cost": "true" if req.cost else "false",
            "otsp": "true" if req.otsp else "false",
            "mode": req.mode,
            "key": LONGDO_API_KEY
        }
        ...
        if resp.status_code == 200:
            ...
            # Parse response - แปลง status เป็น string (อาจเป็น int หรือ string)
            status_raw = result.get("status", "error")
            status = str(status_raw) if status_raw is not None else "error"
            message = result.get("message", "") or ""
            
            # ตรวจสอบว่า status เป็น "200" หรือ 200 (int)
            if (status == "200" or status_raw == 200) and "body" in result:
                body = result["body"]
                sequence = body.get("sequence", [])
                locations = body.get("location", [])
                total_distance = body.get("total_distance", 0)
                total_time = body.get("total_time", 0)
                time_step = body.get("time_step", []) if req.cost else None
                distance_step = body.get("distance_step", []) if req.cost else None
                ...
                return TSPResult(
                    status=status,
                    message=message or "TSP calculation failed"
                )
        else:
            raise HTTPException(resp.status_code, f"TSP API error: {resp.text}")
            
    except HTTPException:
        raise
    except Exception as e:
        print(f"TSP API error: {e}")
        raise HTTPException(500, f"TSP calculation error: {str(e)}")
