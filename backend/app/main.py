...
def optimize_product_layout(vehicle: Vehicle, products: list[dict]) -> LayoutOptimizationResult:
    """วาง Layout สินค้าให้ optimize (3D Bin Packing)"""
    # ประมาณขนาดรถจากประเภท
    vehicle_dims = {
        "6W": {"length": 6.0, "width": 2.2, "height": 2.5},
        "10W": {"length": 9.0, "width": 2.4, "height": 2.8},
        "18W": {"length": 13.0, "width": 2.5, "height": 3.0}
    }
    
    dims = vehicle_dims.get(vehicle.type, {"length": 6.0, "width": 2.2, "height": 2.5})
    
    # First Fit Decreasing Algorithm (simplified 3D bin packing)
    products_sorted = sorted(products, key=lambda p: -(p.get("length", 0) * p.get("width", 0) * p.get("height", 0)))
    ...
    for i, product in enumerate(products_sorted):
        ...
        # ตรวจสอบน้ำหนัก
        if total_weight + p_weight > vehicle.capW:
            warnings.append(f"สินค้า {i+1} น้ำหนักเกินความจุ")
            continue
        
        # ตรวจสอบปริมาตร
        if total_volume + (p_length * p_width * p_height) > vehicle.capV:
            warnings.append(f"สินค้า {i+1} ปริมาตรเกินความจุ")
            continue
        ...
        if current_y + p_width > dims["width"]:
            # ขึ้นชั้นใหม่
            current_y = 0
            current_z += current_row_height
            current_row_height = 0
        
        if current_z + p_height > dims["height"]:
            warnings.append(f"สินค้า {i+1} สูงเกินความจุ")
            continue
        ...
        total_volume += p_length * p_width * p_height
...
def get_longdo_route(waypoints: List[dict]) -> dict:
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            if data.get("code") == 0:
                route = data.get("data", [{}])[0]
                return {
                    "distance_km": route.get("distance", 0) / 1000.0,
                    "duration_seconds": route.get("time", 0),
...
def optimize_layout(payload: LayoutOptimizationRequest, session: Session = Depends(get_session)):
    ...
    return optimize_product_layout(vehicle, payload.products)
