...
VRP_ENGINE = os.getenv("VRP_ENGINE", "ortools")  # ortools | greedy
...
def optimize_plan(payload: PlanIn, session: Session = Depends(get_session)):
    # 1) เตรียมข้อมูลออเดอร์จากลูกค้า
...
def solve_vrp_greedy(origin: dict, orders: list[RouteStop], vehicles: list[Vehicle]) -> list[RouteResult]:
    ...
    while remaining:
        ...
        veh_dict = {
            "plate": veh.plate, "type": veh.type, "capW": veh.capW, "capV": veh.capV,
            "owner": veh.owner, "costPerKm": veh.costPerKm, "status": veh.status
        }
        routes.append(RouteResult(vehicle=veh_dict, distanceKm=dist, cost=cost, orders=seq))
    return routes

# ===== OR-Tools CVRP (น้ำหนัก + ปริมาตร) =====
