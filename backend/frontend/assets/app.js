  ...
  s.innerHTML = `
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
      <div class="metric-card">
        <div class="metric-label">จำนวนเส้นทาง</div>
        <div class="metric-value text-green-700">${totalRoutes} เส้นทาง</div>
      </div>
      <div class="metric-card">
        <div class="metric-label">รวมระยะทาง</div>
        <div class="metric-value text-blue-700">${(plan.totalKm||0).toFixed(2)} km</div>
      </div>
      <div class="metric-card">
        <div class="metric-label">รวมต้นทุน</div>
        <div class="metric-value text-green-700">${(plan.totalCost||0).toFixed(2)} บาท</div>
      </div>
      <div class="metric-card">
        <div class="metric-label">ต้นทุน/กม.</div>
        <div class="metric-value text-purple-700">${avgCostPerKm.toFixed(2)} บาท</div>
      </div>
      <div class="metric-card">
        <div class="metric-label">รวมน้ำหนัก</div>
        <div class="metric-value text-gray-900">${(plan.totalW||0).toFixed(2)} ตัน</div>
      </div>
      <div class="metric-card">
        <div class="metric-label">รวมปริมาตร</div>
        <div class="metric-value text-gray-900">${(plan.totalV||0).toFixed(2)} ลบ.ม.</div>
      </div>
      <div class="metric-card">
        <div class="metric-label">Utilization เฉลี่ย</div>
        <div class="metric-value ${totalUtilization >= 80 ? 'text-green-600' : totalUtilization >= 60 ? 'text-yellow-600' : 'text-red-600'}">${totalUtilization.toFixed(1)}%</div>
      </div>
      <div class="metric-card">
        <div class="metric-label">ระยะทางเฉลี่ย</div>
        <div class="metric-value text-gray-900">${avgDistancePerRoute.toFixed(2)} km</div>
      </div>
    </div>`;
