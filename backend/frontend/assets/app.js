...
async function analyzePlan(){
  try{
    ...
    // ใช้ TSP เพื่อจัดลำดับ orders อัตโนมัติ (ถ้าเปิดใช้งาน)
    let optimizedLines = lines;
    const useTSP = $("#use_tsp") ? $("#use_tsp").checked : false;
    
    if (useTSP && lines.length > 1) {
      try {
        console.log("🔄 กำลังใช้ TSP เพื่อจัดลำดับ orders...");
        
        // ดึงข้อมูลลูกค้าเพื่อหาพิกัด
        ...
        // สร้าง list ของพิกัด (origin + customers)
        const lons = [origin_lng];
        const lats = [origin_lat];
        const lineIndices = [0]; // เก็บ index ของ origin (0)
        
        lines.forEach((line, idx) => {
          const customer = customerMap[line.customer_code];
          if (customer && customer.lng && customer.lat) {
            lons.push(customer.lng);
            lats.push(customer.lat);
            lineIndices.push(idx + 1); // +1 เพราะมี origin อยู่ที่ index 0
          }
        });
        
        // เรียก TSP API ถ้ามีจุดมากกว่า 2 จุด (origin + อย่างน้อย 1 customer)
        if (lons.length > 2) {
          const tspResult = await api("/api/routing/tsp", {
            method: "POST",
            json: {
              lon: lons,
              lat: lats,
              cost: true,  // รวม time_step และ distance_step
              otsp: true,   // กลับจุดเริ่มต้น
              mode: $("#consider_traffic").checked ? "t" : "c"  // t=fastest+traffic, c=fastest static
            }
          });
          
          if (tspResult.status === "200" && tspResult.sequence && tspResult.sequence.length > 0) {
            console.log("✅ TSP optimization สำเร็จ:", tspResult);
            
            // จัดลำดับ lines ตาม sequence จาก TSP (ข้าม index 0 ที่เป็น origin)
            const sequence = tspResult.sequence;
            optimizedLines = [];
            
            // เริ่มจาก index 1 (ข้าม origin ที่ index 0)
            for (let i = 1; i < sequence.length; i++) {
              const seqIndex = sequence[i];
              // ถ้า seqIndex เป็น 0 แสดงว่าเป็น origin (ข้าม)
              if (seqIndex > 0 && seqIndex < lineIndices.length) {
                const lineIndex = lineIndices[seqIndex] - 1; // -1 เพราะ origin อยู่ที่ index 0
                if (lineIndex >= 0 && lineIndex < lines.length) {
                  optimizedLines.push(lines[lineIndex]);
                }
              }
            }
            ...
            const tspInfo = `
              📊 TSP Optimization:
              - ลำดับเดิม: ${lines.length} จุด
              - ลำดับใหม่: ${optimizedLines.length} จุด
              - ระยะทางรวม: ${(tspResult.total_distance / 1000).toFixed(2)} km
              - เวลาเดินทาง: ${Math.floor(tspResult.total_time / 60)} นาที ${tspResult.total_time % 60} วินาที
            `;
            console.log(tspInfo);
            
            // แสดง alert ถ้าต้องการ
            if (optimizedLines.length !== lines.length) {
              console.warn("⚠️ จำนวน orders หลัง TSP ไม่เท่ากับเดิม");
            }
          } else {
            console.warn("⚠️ TSP optimization ไม่สำเร็จ:", tspResult.message);
            optimizedLines = lines; // ใช้ลำดับเดิม
          }
        } else {
          console.log("ℹ️ ไม่มีจุดเพียงพอสำหรับ TSP (ต้องมีอย่างน้อย 2 จุด)");
          optimizedLines = lines;
        }
      } catch(e) {
        console.error("❌ TSP optimization error:", e);
        optimizedLines = lines; // ใช้ลำดับเดิมถ้าเกิด error
      }
    }

    const payload = {
      name, date, origin_lat, origin_lng, lines: optimizedLines,
      plan_type,
...
function initLongdoMap(){
  ...
  // ตรวจสอบและตั้งค่า default values เพื่อป้องกัน null
  const mapOptions = {
    placeholder: mapDiv,
    zoom: 10,
  ...
  if (mapOptions.zoom == null) mapOptions.zoom = 10;
  if (mapOptions.pitch == null) mapOptions.pitch = 45;
  if (mapOptions.lastView == null) mapOptions.lastView = false;
  
  _longdoMap = new longdo.Map(mapOptions);
  
  _longdoMap.Event.bind("ready", function() {
    // เพิ่ม Traffic Layer - ตรวจสอบว่าไม่มี null values
    try {
      if (window.longdo && window.longdo.Layers && _longdoMap && _longdoMap.Layers) {
        if (typeof _longdoMap.Layers.insert === 'function') {
          _longdoMap.Layers.insert(1, window.longdo.Layers.TRAFFIC);
        }
      }
    } catch(e) {
    ...
    if (window.deck && window.deck.ScenegraphLayer) {
      try {
        ...
        // ตรวจสอบว่า data ไม่มี null values
        const validData = data.filter(d => 
          d && d.coordinates && 
          Array.isArray(d.coordinates) && 
          d.coordinates.length >= 3 &&
          d.coordinates.every(c => c != null && !isNaN(Number(c)))
        );
...
async function drawLongdoMap(plan){
  ...
  // จุดเริ่มต้น - เพิ่ม error handling และตรวจสอบ null values
  try {
    // ตรวจสอบว่าพิกัดไม่เป็น null
    if (!plan.origin_lng || !plan.origin_lat || isNaN(plan.origin_lng) || isNaN(plan.origin_lat)) {
    ...
    try {
      ...
      `);
      markerOptions.icon = { url: `data:image/svg+xml;base64,${iconSvg}` };
    } catch(e) {
      console.warn("Error creating icon:", e);
    }
    
    const originMarker = new longdo.Marker(
      { lon: Number(plan.origin_lng), lat: Number(plan.origin_lat) },
      markerOptions
    );
    if (_longdoMap && _longdoMap.Overlays) {
  ...
  for (let idx = 0; idx < (plan.routes || []).length; idx++) {
    ...
    for (let oIdx = 0; oIdx < orders.length; oIdx++) {
      ...
      try {
        ...
        // สร้าง marker options โดยตรวจสอบ null values
        const markerOptions = {
          title: `${o.code || ''} - ${o.name || ''}`,
          detail: `พิกัด: ${Number(o.lat).toFixed(6)}, ${Number(o.lng).toFixed(6)}\nน้ำหนัก: ${(o.w||0).toFixed(2)} ตัน | ปริมาตร: ${(o.v||0).toFixed(2)} ลบ.ม.`
        };
        ...
        try {
          const iconSvg = btoa(`
            <svg width="32" height="32" xmlns="http://www.w3.org/2000/svg">
              <circle cx="16" cy="16" r="13" fill="${color || '#22c55e'}" stroke="white" stroke-width="2" opacity="0.9"/>
              <text x="16" y="20" text-anchor="middle" fill="white" font-size="12" font-weight="bold">${oIdx + 1}</text>
            </svg>
          `);
          markerOptions.icon = { url: `data:image/svg+xml;base64,${iconSvg}` };
        } catch(e) {
          console.warn("Error creating customer icon:", e);
        }
        
        const customerMarker = new longdo.Marker(
          { lon: Number(o.lng), lat: Number(o.lat) },
          markerOptions
        );
        if (_longdoMap && _longdoMap.Overlays) {
    ...
    try {
      if (idx === 0) {
        ...
        if (_longdoMap && _longdoMap.Route) {
          ...
          // เพิ่ม waypoints - ตรวจสอบ null values
          waypoints.forEach(wp => {
            if (wp && wp.lon != null && wp.lat != null && 
                !isNaN(Number(wp.lon)) && !isNaN(Number(wp.lat))) {
              try {
                _longdoMap.Route.add({ 
                  lon: Number(wp.lon), 
                  lat: Number(wp.lat) 
                });
              } catch(e) {
          ...
          // ไม่ต้องตั้งค่า Truck Routing API จาก frontend (จะถูกบล็อกด้วย CORS)
          // ใช้ Route API มาตรฐานแทน
          ...
          // ใช้ RouteServiceTruck API ผ่าน backend proxy สำหรับรถบรรทุก
          try {
            const validWaypoints = waypoints.filter(wp => 
              wp && wp.lon != null && wp.lat != null && 
              !isNaN(Number(wp.lon)) && !isNaN(Number(wp.lat))
            );
            
            if (validWaypoints.length >= 2) {
              // เรียก backend proxy สำหรับ Truck Routing
              const routeResponse = await api("/api/routing/route", {
                method: "POST",
                json: {
                  ...
                  use_truck: true,  // ใช้ Truck Routing API
                  mode: "t"  // t=fastest+traffic (เหมาะสำหรับรถบรรทุก)
                }
              });
              
              if (routeResponse.success && routeResponse.geometry && routeResponse.geometry.length > 0) {
                // วาดเส้นทางจาก geometry ที่ได้จาก Truck Routing API
                try {
                  if (_longdoMap && _longdoMap.Overlays) {
                    // แปลง geometry เป็น polyline
                    const polylineCoords = routeResponse.geometry.map(point => {
                      if (Array.isArray(point) && point.length >= 2) {
                        return { lon: Number(point[0]), lat: Number(point[1]) };
                      } else if (point.lon && point.lat) {
                        return { lon: Number(point.lon), lat: Number(point.lat) };
                      } else if (point.lng && point.lat) {
                        return { lon: Number(point.lng), lat: Number(point.lat) };
                      }
                      return null;
                    }).filter(p => p !== null);
                    ...
                    if (polylineCoords.length > 0) {
                      ...
                      _routeResults.push({
                        ...
                        waypoints: waypoints,
                        routeInfo: {
                          distance: routeResponse.distance_km,
                          duration: routeResponse.duration_seconds,
                          geometry: routeResponse.geometry
                        },
                        color: color
                      });
                      
                      console.log(`✅ Truck Route ${idx + 1}: ${routeResponse.distance_km.toFixed(2)} km, ${Math.floor(routeResponse.duration_seconds / 60)} นาที`);
                    }
                  }
                } catch(polyErr) {
                  console.warn("Error drawing truck route polyline:", polyErr);
                  // Fallback: ใช้ simple polyline
                  drawSimplePolyline(validWaypoints, color);
                }
              } else {
                console.warn("Truck Route API failed, using fallback:", routeResponse.error);
                // Fallback: ใช้ simple polyline
                drawSimplePolyline(validWaypoints, color);
              }
            } else {
            ...
            console.warn("Truck Route API error:", e);
            // Fallback: ใช้ simple polyline
            const validWaypoints = waypoints.filter(wp => 
              wp && wp.lon != null && wp.lat != null && 
              !isNaN(Number(wp.lon)) && !isNaN(Number(wp.lat))
          ...
          // Helper function สำหรับวาด simple polyline
          function drawSimplePolyline(waypoints, routeColor) {
            if (waypoints.length > 0 && _longdoMap && _longdoMap.Overlays) {
              try {
                const polyline = new longdo.Polyline(
                  waypoints.map(wp => ({ lon: Number(wp.lon), lat: Number(wp.lat) })),
                  { lineColor: routeColor || '#22c55e', lineWidth: 3, opacity: 0.7 }
                );
                _longdoMap.Overlays.add(polyline);
              } catch(polyErr) {
                console.warn("Error adding fallback polyline:", polyErr);
              }
            }
          }
        }
        ...
        try {
          ...
          if (validWaypoints.length > 0 && _longdoMap && _longdoMap.Overlays) {
            ...
            // ตรวจสอบว่าไม่มี null values
            if (polylineOptions.lineColor == null) polylineOptions.lineColor = '#22c55e';
            if (polylineOptions.lineWidth == null) polylineOptions.lineWidth = 3;
            if (polylineOptions.opacity == null) polylineOptions.opacity = 0.7;
            
            const polyline = new longdo.Polyline(
              validWaypoints.map(wp => ({ lon: Number(wp.lon), lat: Number(wp.lat) })),
              polylineOptions
            );
            _longdoMap.Overlays.add(polyline);
          }
  ...
  try {
    if (plan.routes && plan.routes.length > 0 && _longdoMap) {
      ...
      if (allLons.length > 1 && allLats.length > 1) {
        // กรอง null/undefined values
        const validLons = allLons.filter(lon => lon != null && !isNaN(Number(lon)));
        const validLats = allLats.filter(lat => lat != null && !isNaN(Number(lat)));
        
        if (validLons.length > 0 && validLats.length > 0) {
          ...
          // ตรวจสอบว่าไม่มี null/NaN values
          if (!isNaN(minLon) && !isNaN(maxLon) && !isNaN(minLat) && !isNaN(maxLat)) {
            ...
            if (!isNaN(centerLon) && !isNaN(centerLat) && 
                _longdoMap.location && typeof _longdoMap.location === 'function') {
              try {
                _longdoMap.location({ lon: Number(centerLon), lat: Number(centerLat) }, true);
              } catch(e) {
            ...
            if (!isNaN(zoomLevel) && _longdoMap.zoom && typeof _longdoMap.zoom === 'function') {
              try {
                _longdoMap.zoom(Number(zoomLevel), true);
              } catch(e) {
    ...
    } else if (_longdoMap && plan.origin_lng != null && plan.origin_lat != null && 
               !isNaN(Number(plan.origin_lng)) && !isNaN(Number(plan.origin_lat))) {
      // ถ้าไม่มีเส้นทาง ให้ไปยังจุดเริ่มต้น
      if (_longdoMap.location && typeof _longdoMap.location === 'function') {
        try {
          _longdoMap.location({ 
            lon: Number(plan.origin_lng), 
            lat: Number(plan.origin_lat) 
          }, true);
        } catch(e) {
...
async function displayRouteDetail(route, routeIndex){
  const plan = _currentPlanResult;
  ...
  if (_currentRouteMapLayer === 'longdo') {
    await drawLongdoRouteDetail(plan, route, routeIndex);
  }
}
...
async function drawLongdoRouteDetail(plan, route, routeIndex){
  ...
  if (!_routeDetailLongdoMap) {
    ...
    _routeDetailLongdoMap.Event.bind("ready", async function() {
      if (window.longdo && window.longdo.Layers) {
      ...
      // ใช้ RouteServiceTruck API ผ่าน backend proxy สำหรับรถบรรทุก
      try {
        const validWaypoints = _routeWaypoints.filter(wp => 
          wp && wp.lon != null && wp.lat != null && 
          !isNaN(Number(wp.lon)) && !isNaN(Number(wp.lat))
        );
        ...
        if (validWaypoints.length >= 2) {
          ...
          if (routeResponse.success && routeResponse.geometry && routeResponse.geometry.length > 0) {
            // วาดเส้นทางจาก geometry ที่ได้จาก Truck Routing API
            try {
              if (_routeDetailLongdoMap && _routeDetailLongdoMap.Overlays) {
                // แปลง geometry เป็น polyline
                const polylineCoords = routeResponse.geometry.map(point => {
                  if (Array.isArray(point) && point.length >= 2) {
                    return { lon: Number(point[0]), lat: Number(point[1]) };
                ...
                if (polylineCoords.length > 0) {
                  ...
                  _routeDetailLongdoMap.Overlays.add(polyline);
                  
                  // แสดงข้อมูลเส้นทางใน resultDiv
                  if (resultDiv) {
                    resultDiv.innerHTML = `
                      <div class="p-3 bg-green-50 rounded-lg border border-green-200">
                        <div class="text-sm font-semibold text-green-800 mb-2">🚚 เส้นทางรถบรรทุก</div>
                        <div class="text-xs text-gray-700 space-y-1">
                          <div>ระยะทาง: <strong>${routeResponse.distance_km.toFixed(2)} km</strong></div>
                          <div>เวลาเดินทาง: <strong>${Math.floor(routeResponse.duration_seconds / 60)} นาที ${routeResponse.duration_seconds % 60} วินาที</strong></div>
                          <div class="mt-2 text-green-700">✅ เส้นทางเหมาะสมสำหรับรถบรรทุก (หลีกเลี่ยงถนนแคบ, สะพานต่ำ, ทางลาดชัน)</div>
                        </div>
                      </div>
                    `;
                  }
                  
                  console.log(`✅ Truck Route Detail: ${routeResponse.distance_km.toFixed(2)} km, ${Math.floor(routeResponse.duration_seconds / 60)} นาที`);
                }
              }
            } catch(polyErr) {
              console.warn("Error drawing truck route polyline:", polyErr);
              drawSimplePolylineForDetail(validWaypoints);
            }
          } else {
            console.warn("Truck Route API failed, using fallback:", routeResponse.error);
            drawSimplePolylineForDetail(validWaypoints);
        ...
        console.warn("Truck Route API error:", e);
        drawSimplePolylineForDetail(_routeWaypoints.filter(wp => 
          wp && wp.lon != null && wp.lat != null && 
          !isNaN(Number(wp.lon)) && !isNaN(Number(wp.lat))
        ));
      }
      
      // Helper function สำหรับวาด simple polyline ใน detail view
      function drawSimplePolylineForDetail(waypoints) {
        if (waypoints.length > 0 && _routeDetailLongdoMap && _routeDetailLongdoMap.Overlays) {
          try {
            const polyline = new longdo.Polyline(
              waypoints.map(wp => ({ lon: Number(wp.lon), lat: Number(wp.lat) })),
              { lineColor: '#22c55e', lineWidth: 3, opacity: 0.7 }
            );
            _routeDetailLongdoMap.Overlays.add(polyline);
          } catch(polyErr) {
            console.warn("Error adding fallback polyline:", polyErr);
      ...
      if (wp && wp.lon != null && wp.lat != null && 
        ...
        try {
          if (wp.type === 'origin' || wp.type === 'customer') {
            const marker = new longdo.Marker(
              { lon: Number(wp.lon), lat: Number(wp.lat) },
              { title: wp.label, detail: wp.order ? `น้ำหนัก: ${(wp.order.w||0).toFixed(2)} ตัน` : '' }
            );
            _routeDetailLongdoMap.Route.add(marker);
          } else {
            _routeDetailLongdoMap.Route.add({ lon: Number(wp.lon), lat: Number(wp.lat) });
          }
        } catch(e) {
          console.warn("Error adding waypoint:", wp, e);
        }
...
async function switchRouteMapLayer(type){
  ...
  if (type === 'longdo') {
    ...
    if (_currentPlanResult && _selectedRouteIndex !== null) {
      const route = _currentPlanResult.routes[_selectedRouteIndex];
      if (route) {
        await drawLongdoRouteDetail(_currentPlanResult, route, _selectedRouteIndex);
      }
    }
  } else {
    if (_routeDetailLongdoMap) {
