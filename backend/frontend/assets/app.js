...
async function addProduct(){
  try {
    ...
    const stackable = $("#product_stackable").checked;
  ...
  try {
    ...
    for (const product of defaultProducts) {
      try {
        ...
        // ถ้ามีอยู่แล้วข้าม
        if (e.message && (e.message.includes("already exists") || e.message.includes("403"))) {
          continue;
        }
...
function openProductManager(){
  // เปิด modal หรือ tab สำหรับจัดการสินค้า
  alert("ฟีเจอร์จัดการสินค้าจะเปิดในหน้าต่างใหม่\n\nสามารถเพิ่ม/แก้ไข/ลบสินค้าได้ที่ API: /products");
  // TODO: สร้าง modal หรือหน้าใหม่สำหรับจัดการสินค้า
}
...
async function renderPlanResult(plan){
  ...
  s.innerHTML = `
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="metric p-4 bg-white border-2 border-green-200 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">จำนวนเส้นทาง</div>
        <div class="v text-xl font-bold text-green-700">${totalRoutes} เส้นทาง</div>
      </div>
      <div class="metric p-4 bg-white border-2 border-blue-200 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">รวมระยะทาง</div>
        <div class="v text-xl font-bold text-blue-700">${(plan.totalKm||0).toFixed(2)} km</div>
      </div>
      <div class="metric p-4 bg-white border-2 border-green-300 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">รวมต้นทุน</div>
        <div class="v text-xl font-bold text-green-700">${(plan.totalCost||0).toFixed(2)} บาท</div>
      </div>
      <div class="metric p-4 bg-white border-2 border-purple-200 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">ต้นทุน/กม.</div>
        <div class="v text-xl font-bold text-purple-700">${avgCostPerKm.toFixed(2)} บาท</div>
      </div>
      <div class="metric p-4 bg-white border-2 border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">รวมน้ำหนัก</div>
        <div class="v text-xl font-bold text-gray-900">${(plan.totalW||0).toFixed(2)} ตัน</div>
      </div>
      <div class="metric p-4 bg-white border-2 border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">รวมปริมาตร</div>
        <div class="v text-xl font-bold text-gray-900">${(plan.totalV||0).toFixed(2)} ลบ.ม.</div>
      </div>
      <div class="metric p-4 bg-white border-2 ${totalUtilization >= 80 ? 'border-green-300' : totalUtilization >= 60 ? 'border-yellow-300' : 'border-red-300'} rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">Utilization เฉลี่ย</div>
        <div class="v text-xl font-bold ${totalUtilization >= 80 ? 'text-green-600' : totalUtilization >= 60 ? 'text-yellow-600' : 'text-red-600'}">${totalUtilization.toFixed(1)}%</div>
      </div>
      <div class="metric p-4 bg-white border-2 border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <div class="k text-xs mb-2 text-gray-600 font-semibold uppercase tracking-wide">ระยะทางเฉลี่ย</div>
        <div class="v text-xl font-bold text-gray-900">${avgDistancePerRoute.toFixed(2)} km</div>
      </div>
    </div>`;
...
async function drawLongdoMap(plan){
  ...
  for (let idx = 0; idx < (plan.routes || []).length; idx++) {
    ...
    if (longdo.RouteMode && typeof _longdoMap.Route.mode === 'function') {
      try {
    ...
    // if (longdo.RouteType && typeof _longdoMap.Route.type === 'function') {
    //   try {
  ...
  // Fit bounds - ใช้ bound() เพื่อแสดงขอบเขตที่ครอบคลุมทุกจุด
