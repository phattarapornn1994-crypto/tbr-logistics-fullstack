      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th>รวมน้ำหนัก</th>
            <th>รวมปริมาตร</th>
            <th>Utilization เฉลี่ย</th>
            <th>ระยะทางเฉลี่ย</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td class="font-bold text-green-700">${totalRoutes} เส้นทาง</td>
            <td class="font-bold text-blue-700">${(plan.totalKm||0).toFixed(2)} km</td>
            <td class="font-bold text-green-700">${(plan.totalCost||0).toFixed(2)} บาท</td>
            <td class="font-bold text-purple-700">${avgCostPerKm.toFixed(2)} บาท</td>
            <td class="font-bold text-gray-900">${(plan.totalW||0).toFixed(2)} ตัน</td>
            <td class="font-bold text-gray-900">${(plan.totalV||0).toFixed(2)} ลบ.ม.</td>
            <td class="font-bold ${totalUtilization >= 80 ? 'text-green-600' : totalUtilization >= 60 ? 'text-yellow-600' : 'text-red-600'}">${totalUtilization.toFixed(1)}%</td>
            <td class="font-bold text-gray-900">${avgDistancePerRoute.toFixed(2)} km</td>
          </tr>
        </tbody>
      </table>
    </div>`;
...
async function drawLongdoMap(plan){
  ...
  for (let idx = 0; idx < (plan.routes || []).length; idx++) {
    ...
    orders.forEach((o, oIdx) => {
      ...
      // เพิ่ม marker สำหรับลูกค้า - แสดงพิกัดและข้อมูล
    ...
    // ใช้ Route API เพื่อวาดเส้นทาง - วาดแต่ละเส้นทางแยกกัน
    if (idx === 0) {
      ...
      const polyline = new longdo.Polyline(
        waypoints.map(wp => ({ lon: wp.lon, lat: wp.lat })),
        {
          lineColor: color,
          lineWidth: 3,
          opacity: 0.7
