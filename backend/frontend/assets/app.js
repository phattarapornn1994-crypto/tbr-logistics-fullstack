    ...
    if (tb) {
      ...
      data.forEach(p => {
        ...
        tr.innerHTML = `
          ...
          <td class="text-xs">${p.weight_kg} kg/${p.unit || 'ก้อน'}</td>
  ...
  s.innerHTML = `
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="metric p-3">
        <div class="k text-xs mb-1">จำนวนเส้นทาง</div>
        <div class="v text-base font-bold">${totalRoutes} เส้นทาง</div>
      </div>
      <div class="metric p-3">
        <div class="k text-xs mb-1">รวมระยะทาง</div>
        <div class="v text-base font-bold">${(plan.totalKm||0).toFixed(2)} km</div>
      </div>
      <div class="metric p-3">
        <div class="k text-xs mb-1">รวมต้นทุน</div>
        <div class="v text-base font-bold text-green-700">${(plan.totalCost||0).toFixed(2)} บาท</div>
      </div>
      <div class="metric p-3">
        <div class="k text-xs mb-1">ต้นทุน/กม.</div>
        <div class="v text-base font-bold">${avgCostPerKm.toFixed(2)} บาท</div>
      </div>
      <div class="metric p-3">
        <div class="k text-xs mb-1">รวมน้ำหนัก</div>
        <div class="v text-base font-bold">${(plan.totalW||0).toFixed(2)} ตัน</div>
      </div>
      <div class="metric p-3">
        <div class="k text-xs mb-1">รวมปริมาตร</div>
        <div class="v text-base font-bold">${(plan.totalV||0).toFixed(2)} ลบ.ม.</div>
      </div>
      <div class="metric p-3">
        <div class="k text-xs mb-1">Utilization เฉลี่ย</div>
        <div class="v text-base font-bold ${totalUtilization >= 80 ? 'text-green-600' : totalUtilization >= 60 ? 'text-yellow-600' : 'text-red-600'}">${totalUtilization.toFixed(1)}%</div>
      </div>
      <div class="metric p-3">
        <div class="k text-xs mb-1">ระยะทางเฉลี่ย</div>
        <div class="v text-base font-bold">${avgDistancePerRoute.toFixed(2)} km</div>
      </div>
    </div>`;
