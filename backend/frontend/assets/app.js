...
async function renderPlanResult(plan){
  ...
  s.innerHTML = `
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div class="metric">
        <div class="k">ระยะทางเฉลี่ย/เส้นทาง</div>
  ...
  for (let i = 0; i < (plan.routes || []).length; i++) {
    ...
    summaryDiv.innerHTML = `
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs mb-3 pt-3 border-t border-gray-200">
        ...
        <div>
          <span class="font-semibold">ต้นทุน/ตัน:</span> ${costPerTon.toFixed(2)} บาท
        </div>
        <div>
          <span class="font-semibold">ต้นทุน/กม.:</span> ${costPerKm.toFixed(2)} บาท
        </div>
        <div>
